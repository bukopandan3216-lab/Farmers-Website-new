...
if (typeof apiGet === 'undefined') {
  window.apiGet = async function apiGet(url) {
    try {
      ...
      if (!response.ok) {
        ...
        return { success: false, message: text || 'Server request failed.' };
      }
      ...
      try {
        ...
        console.error('Admin invalid JSON response:', error, text);
  ...
  console.log('[admin.js] fallback apiGet installed');
}

if (typeof getImageSrc === 'undefined') {
  window.getImageSrc = function getImageSrc(path) {
    if (!path) return 'https://images.unsplash.com/photo-1606787366850-de6330128bfc?w=400&q=80';
    return typeof path === 'string' && path.startsWith('http') ? path : path;
  };
}

if (typeof PLACEHOLDER_IMG === 'undefined') {
  window.PLACEHOLDER_IMG = 'https://images.unsplash.com/photo-1606787366850-de6330128bfc?w=400&q=80';
}
  ...
  document.getElementById('admin-users-list').innerHTML = users.map(user => `
    <tr style="border-bottom:1px solid #e5e7eb;">
      ...
      <td style="padding:12px;">
        <span style="background:${user.status === 'active' ? '#ecfdf5' : user.status === 'pending' ? '#fef3c7' : '#fee2e2'}; color:${user.status === 'active' ? '#047857' : user.status === 'pending' ? '#92400e' : '#991b1b'}; padding:4px 12px; border-radius:6px; font-weight:600; font-size:12px;">${(user.status || 'unknown').charAt(0).toUpperCase() + (user.status || 'unknown').slice(1)}</span>
      </td>
...
async function showUserDetailsModal(userId, action) {
  ...
  // Populate modal
  document.getElementById('user-modal-name').textContent = user.full_name || 'Unknown';
  document.getElementById('user-modal-email').textContent = user.email || 'Unknown';
  document.getElementById('user-modal-role').textContent = user.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : 'Unknown';
  const statusText = (user.status || 'unknown').toString().trim().toLowerCase();
  document.getElementById('user-modal-status').textContent = statusText.charAt(0).toUpperCase() + statusText.slice(1);
  document.getElementById('user-modal-date').textContent = user.created_at ? new Date(user.created_at).toLocaleDateString() : 'Unknown';
